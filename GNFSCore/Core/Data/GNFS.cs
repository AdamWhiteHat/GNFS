...
namespace GNFSCore.Core.Data
{
	...
	public partial class GNFS
	{
		...
		[DataMember]
		public BigInteger SquareRoot_Progress_InertPrime_LastValue { get; set; }

		public SquareRoot SquareRoot { get; set; }
			...
			: this()
		{
			...
			if (createdNewData || !Directory.Exists(SaveLocations.SaveDirectory))
			{
				...
				if (polyDegree == -1)
				{
					PolynomialDegree = CalculateDegree(n);
				}
				else
				{
				...
				ConstructNewPolynomial(PolynomialBase, PolynomialDegree);
				LogMessage($"Polynomial constructed: {CurrentPolynomial}");
				LogMessage($"Polynomial base: {PolynomialBase}");
				...
				SquareRoot_Progress_InertPrime_LastValue = QuadraticFactorPairCollection.Last().P;
		...
		public void CaclulatePrimeFactorBaseBounds(BigInteger bound)
		{
			...
			PrimeFactorBase.QuadraticBaseCount = CalculateQuadraticBaseSize(PrimeFactorBase, PolynomialDegree);

			PrimeFactorBase.QuadraticFactorBaseMin = PrimeFactorBase.AlgebraicFactorBaseMax + 20;
			PrimeFactorBase.QuadraticFactorBaseMax = PrimeFactory.GetApproximateValueFromIndex((ulong)(PrimeFactorBase.QuadraticFactorBaseMin + PrimeFactorBase.QuadraticBaseCount));
		...
		private static int CalculateQuadraticBaseSize(FactorBase primeFactorBase, int polyDegree)
		{
			int result = 0;
			/*			
		...
		private void NewFactorPairCollections(CancellationToken cancelToken)
		{
			...
			{
				RationalFactorPairCollection = Factory.BuildRationalFactorPairCollection(this);
			}
			...
			{
				AlgebraicFactorPairCollection = Factory.BuildAlgebraicFactorPairCollection(cancelToken, this);
			}
			...
			{
				QuadraticFactorPairCollection = Factory.BuildQuadraticFactorPairCollection(cancelToken, this);
			}
